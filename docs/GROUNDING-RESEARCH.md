...
Output of a 19-agent research + adversarial-verification pass (2026-06-15). Every load-bearing
...
- ⚠️ **FLAGGED** — plausible but only secondary sources reached; do not hard-code without confirming.
...
The prime directive for this project is **zero hallucination**. Numbers below carry their source
...
✅ **matplotlib `RdBu` midpoint is light gray #F7F7F7 = (247,247,247), NOT pure white.**
...
✅ **QuTiP `plot_wigner` default = `cm.RdBu` under a symmetric norm `Normalize(-max|W|, +max|W|)`.**
...
✅ **QuTiP quadrature convention: g = √2, α = ½·g·(x+iy) = (x+iy)/√2, ħ = 1.** Our validated core
...
✅ **max|W| for the vacuum / any coherent state = 1/π.** Confirms `w_max = 1/π` as the physical
...
✅ **Single-excitation Tavis-Cummings = an (M+1)×(M+1) arrowhead matrix.** Basis
...
✅ **Collective vacuum Rabi splitting Ω_R = 2·g·√M** (full peak-to-peak, on resonance, identical
emitters; half-splitting = g√M). Off-resonance: √(Δ² + 4g²M). Reduces to the 2g Jaynes-Cummings
splitting at M=1. **Convention trap (flagged by the skeptic):** the factor of 2 holds only when g
...
✅ **Exactly M−1 dark states + 2 bright polaritons** in the single-excitation manifold of M
...
✅ **Hopfield coefficients:** |C|² (photon) + |X|² (matter) = 1; at zero detuning each polariton is
...
✅ **Model:** Gaussian (diagonal/energetic) disorder — emitter frequencies ω_i ~ N(ω₀, σ²). Slider
...
✅ **Polaritons are robust; the gap slightly *widens* with disorder** by ≈ 2σ²/(√N·|Ṽ|) ~ σ²/Ω_R
(it does *not* shrink). → *Source:* arXiv:2202.06643.

✅ **Dark states "turn grey":** they borrow oscillator strength / photonic character as σ grows
...
⚠️ **Delocalization criterion Ω_R/(4σ) ≥ 1 (full), Ω_R/(3σ) ≥ 1 (partial).** The *ratio* governs
...
✅ **Motional/exchange narrowing (Houdré 1996):** collective coupling to one shared mode averages
out inhomogeneous broadening, so the polariton linewidth can be *narrower* than the bare
distribution and the splitting size is unchanged. ⚠️ The Houdré 1996 full text (PRA 53, 2711) was
...
❌ **Scope correction — Sharma & Chen 2024 (arXiv:2406.17101) is SINGLE-MODE with HOMOGENEOUS
...
## §4 — Cavity geometry → κ map (the honest configurator)
...
| amplitude / HWHM rate | κ/2 ≈ c(1−R)/(2nL) | — |
...
🔬 The 1/(2nL)-vs-1/(nL) prefactor, and whether one or both mirrors are lossy, and whether L is the
...
## §5 — Material presets (literature defaults 🔬 pending Shravan)
...
|---|---|---|---|---|
| **Organic** microcavity (J-aggregate/dye) | full Ω_R ≈ 80–265 meV (Lidzey 1998: **160 meV**; up to ~500 meV) | ⚠️ ~10–100 (metal), ~325 (DBR) | ⚠️ ~40 meV | nature.com/articles/25692; Lidzey 1999 PRL 82, 3316 |
...
⚠️ Organic Q (~100) and exciton γ (~40 meV) came from secondary summaries — confirm against a
...
❌ **Correction — neither cited paper has an angle-resolved dispersion plot.** "Lagoudakis 2017
...
✅ **Dispersion plot:** Energy (**eV**) on y; angle θ (deg) or in-plane wavevector k∥ on x
...
✅ **Intensity maps:** reflectivity/PL shown as a sequential colormap (modern: viridis; historic:
...
All ✅ from the APS PRL "Information for Contributors" + Physical Review Style Guide (Wayback
...
- ⚠️ APS does **not** mandate a figure font family (only "consistent with body text" = Computer
