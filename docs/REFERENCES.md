...
field's killer figure is energy-vs-bond-length LP/UP curves with **dashed bare references, solid
...
### C.1 Canonical angle-resolved dispersion (UP/LP anticrossing) — OUR HERO
...
**Lesson.** The genre-defining figure and our **promoted centerpiece** (`dispersionVsAngle`): four
...
### C.2 Canonical "panel-a" cavity geometry schematic — OUR GEOMETRY VIEW
...
SPP model** (the plasmonic-toggle dispersion + two-colormap field-map rule), the **Berloff/Berloff
