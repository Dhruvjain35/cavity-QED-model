...
import { DBRMirror, HALF, LENGTH, LightRig, N_ANTINODES, STAGE_OFFSET, STAGE_SCALE, W0 } from "./cavityKit";
...
function buildFilm(ens: Ens) {
  ...
  const mols = ens.dipoles.map((d, i) => {
    ...
    return {
      pos: new THREE.Vector3(Math.cos(th) * rad * CLUSTER_R, y * CLUSTER_R, Math.sin(th) * rad * CLUSTER_R),
    ...
    <group>
      {film.mols.map((mol, i) => (
        <group key={i} position={mol.pos} scale={scale}>
          <mesh>
            <sphereGeometry args={[MOL_R, 24, 24]} />
            <meshStandardMaterial ref={(el) => { mats.current[i] = el; }} color={C_GROUND} emissive={C_GROUND} emissiveIntensity={0.12} roughness={0.38} metalness={0.0} />
          </mesh>
        </group>
      ))}
    </group>
...
function StandingWaveField({ ampRef, intensity = 1, visible = true }: { ampRef: MutableRefObject<number>; intensity?: number; visible?: boolean }) {
  ...
  const k = (N_ANTINODES * Math.PI) / LENGTH;
  const geo = useMemo(() => {
    const pts: THREE.Vector3[] = [], SEG = 280;
    for (let i = 0; i <= SEG; i++) { const z = -HALF + (i / SEG) * LENGTH; pts.push(new THREE.Vector3(0, Math.sin(k * (z + HALF)) * WAVE_AMP, z)); }
    return new THREE.TubeGeometry(new THREE.CatmullRomCurve3(pts), SEG, 1.6, 14, false);
  }, [k]);
  const antinodeZ = useMemo(() => Array.from({ length: N_ANTINODES }, (_, j) => -HALF + ((2 * j + 1) * HALF) / N_ANTINODES), []);
  useFrame(() => { const g = grp.current; if (g) g.scale.y = Math.max(0.0015, Math.sqrt(Math.max(0, Math.min(1, ampRef.current))) * intensity); });
    ...
    <group>
      ...
      {antinodeZ.map((z, j) => (
        <mesh key={j} position={[0, 0, z]}>
          ...
          <meshBasicMaterial color="#1b6f8c" transparent opacity={0.18} depthWrite={false} toneMapped={false} />
        </mesh>
      ))}
      <group ref={grp}>
    ...
    <div className="cav-stage">
      ...
      <Canvas dpr={[1, 2]} gl={{ antialias: true, alpha: true }} camera={{ position: [0, 60, 330], fov: 50 }}>
        ...
        <group rotation={LIVE_TILT} scale={STAGE_SCALE} position={STAGE_OFFSET}>
          <DBRMirror side={-1} />
          <DBRMirror side={1} />
          <StandingWaveField ampRef={fieldAmpRef} visible={controls.showFieldDiscs} intensity={controls.fieldGlow} />
